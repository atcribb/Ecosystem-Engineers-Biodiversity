...
for(i in 1:length(stage_names)){
  ...
  if(nrow(all_EE_data>0)){ # don't get stuck on reef gaps
    ...
    for(j in 1:iter){
      ...
      n1 <- length(unique(presence_formations)) #number of formations containing reef-building ecosystem engineers
      n2 <- length(unique(absence_formations)) #number of formations NOT containing reef-building ecosystem engineers
    ...
    #When there n>1
    #Weighted means and standard deviation for EE (m1) and non-EE (m2) data to downweight those iterations with large errors  (aka where we've subsampled formations with large varianges in measurements)
    #Unweighted means and standard deviation for Hedges g, which lacks a standard deviation for each iteration because it takes the m1 and m2 standard deviations to calculate 
    #But when there is only one or no formation (n<2), cannot take weighted mean because there no standard deviation
    #n2 is never going to be too small for weighted means, as long as there is a fossil record 
    #This is really only very an issue where we have true reef gaps in the Cambrian, so we can just deal with n1 here:
    
    if(n1>1){
      #Richness
      m1_richness_weights <- 1/( (M1_genrich_sd_iters/sqrt(iter)) ^2) #weights for each iteration =  1/(SE^2)
      results_df$M1_genrich[i] <- wtd.mean(M1_genrich_iters, weights=m1_richness_weights, normwt=TRUE, na.rm=TRUE) 
      results_df$M1_genrich_sd[i] <- sqrt(wtd.var(M1_genrich_iters, weights=m1_richness_weights, normwt=TRUE, na.rm=TRUE))
      
      #Shannon's Diversity
      m1_H_weights <- 1/( (M1_H_sd_iters/sqrt(iter)) ^2) #weights for each iteration =  1/(SE^2)
      results_df$M1_H[i] <- wtd.mean(M1_H_iters, weights=m1_H_weights, normwt=TRUE, na.rm=TRUE) 
      results_df$M1_H_sd[i] <- sqrt(wtd.var(M1_H_iters, weights=m1_H_weights, normwt=TRUE, na.rm=TRUE))
      
      #Simpson's Dominance
      m1_dom_weights <- 1/( (M1_dom_sd_iters/sqrt(iter))^2) #weights for each iteration = 1/(SE^2)
      results_df$M1_dom[i] <- wtd.mean(M1_dom_iters, weights=m1_dom_weights, normwt=TRUE, na.rm=TRUE)
      results_df$M1_dom_sd[i] <- sqrt(wtd.var(M1_dom_iters, weights=m1_dom_weights, normwt=TRUE, na.rm=TRUE))
    ...
    if(n2>1){ 
      ...
      #Richness
      m2_richness_weights <- 1/( (M2_genrich_sd_iters/sqrt(iter)) ^2) #weights for each iteration = 1/(SE^)
      results_df$M2_genrich[i] <- wtd.mean(M2_genrich_iters, weights=m2_richness_weights, normwt=TRUE, na.rm=TRUE) 
      results_df$M2_genrich_sd[i] <- sqrt(wtd.var(M2_genrich_iters, weights=m2_richness_weights, normwt=TRUE, na.rm=TRUE))
      
      #Shannon's Diversity 
      m2_H_weights <- 1/( (M2_H_sd_iters/sqrt(iter)) ^2) #weights for each iteration =  1/(SE^2)
      results_df$M2_H[i] <- wtd.mean(M2_H_iters, weights=m2_H_weights, normwt=TRUE, na.rm=TRUE) 
      results_df$M2_H_sd[i] <- sqrt(wtd.var(M2_H_iters, weights=m2_H_weights, normwt=TRUE, na.rm=TRUE))
      
      #Simpson's Dominance
      m2_dom_weights <- 1/( (M2_dom_sd_iters/sqrt(iter))^2) #weights for each iteration = 1/(SE^2)
      results_df$M2_dom[i] <- wtd.mean(M2_dom_iters, weights=m2_dom_weights, normwt=TRUE, na.rm=TRUE)
      results_df$M2_dom_sd[i] <- sqrt(wtd.var(M2_dom_iters, weights=m2_dom_weights, normwt=TRUE, na.rm=TRUE))
    ...
    #Hedges G - unweighted mean and standard deviation 
    #Generic richness
