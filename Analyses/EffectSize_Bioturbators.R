...
#Summary: Calculates effect size of bioturbating ecosystem engineers on generic richness, Shannon's Diversity, and Simpson's Dominance for each stage through the Phanerozoic
...
for(i in 1:length(stage_names)){
  ...
  if(nrow(all_EE_data>0)){ # don't get stuck on time periods with no ecosystem engineers
    ...
    for(j in 1:iter){
      ...
      if(n1>0){ #don't get stuck on time periods where there are 0 ecosystem engineering formations
    ...
    #When there n>1
    #Weighted means and standard deviation for EE (m1) and non-EE (m2) data to downweight those iterations with large errors  (aka where we've subsampled formations with large variances in measurements)
    ...
    #But when there is only one or no formation, cannot take weighted mean because there no standard deviation
